...
from models.KAN.layers import Encoder, IEAM, KEAM
...
class VQATransformer(tf.keras.Model):
    def __init__(self, num_layers, d_model, num_heads, dff, vocab_size, pe_input, pe_target,
        ...
        self.keam = KEAM(num_layers, d_model, num_heads, dff, pe_target, rate)
        self.img_dense = Dense(d_model, activation='relu')
        self.kn_dense = Dense(d_model, activation='relu')
        self.fc1 = Dense(3 * multiplier)
        self.fc2 = Dense(3 * multiplier)
        self.final_layer = tf.keras.layers.Dense(vocab_size)

    def call(self, question, img, kn, tar, training, enc_padding_mask, kn_padding_mask, dec_padding_mask):
        ...
        dec_output1 = tf.keras.layers.Reshape((2, -1))(dec_output1) # (64, 3, 490)
        ...
        dec_output2 = tf.keras.layers.Reshape((2, -1))(dec_output2) # (64, 3, 1550)
        dec_output2 = self.kn_dense(dec_output2) # (64, 3, 512)

        final_output = self.final_layer(dec_output1+dec_output2)  # (batch_size, tar_seq_len, target_vocab_size)
