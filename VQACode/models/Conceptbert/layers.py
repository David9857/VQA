...
class EncoderLayer(tf.keras.layers.Layer):
    ...
    def call(self, question, img, training, mask):
        # (v,k,q,mask)
        attn_output, _ = self.mha(img, img, question, mask)  # (batch_size, input_seq_len, d_model)
...
class Encoder(tf.keras.layers.Layer):
    ...
    def call(self, question, kg, training, mask):
        ...
        for i in range(self.num_layers):
            x = self.enc_layers[i](x, kg, training, None)
