...
class Knowledge_Question_Encoder(tf.keras.layers.Layer):
    def __init__(self, num_layers, d_model, num_heads, dff, maximum_position_encoding,
                 rate=0.1 ,pretrained_cnn_type='inception', multiplier = 10):
        super(Knowledge_Question_Encoder, self).__init__()
        self.maximum_position_encoding = maximum_position_encoding
        # self.pretrained_CNN = pretrained_cnn(pretrained_cnn_type)
        self.fc1 = Dense(maximum_position_encoding * multiplier)
        ...
        self.concatenator = Concatenate(axis=1)
        # self.conv = tf.keras.layers.Conv2D(1024, 3, padding='same', activation='relu')
        self.bilstm1 = tf.keras.layers.Bidirectional(tf.keras.layers.LSTM(512, return_sequences=True))
        self.bilstm2 = tf.keras.layers.Bidirectional(tf.keras.layers.LSTM(512, return_sequences=True))

    def call(self, question, kn, training, mask):  
        question = self.bilstm1(question) # (64, 38, 1024)
        kn = self.bilstm2(kn) # (64, 155, 1024)
        #####
        kn = self.fc1(kn)  # (49, pe_input) (64, 155, 380)
        kn = tf.transpose(kn, perm=(0, 2, 1))  # (Res, 49) (64, 380, 155)
