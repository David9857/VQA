...
from models.Transformer.layers_general import BC_Encoder, Image_Question_Encoder, Knowledge_Question_Encoder, Decoder
...
class VQATransformer(tf.keras.Model):
    ...
    def call(self, question, img, tar, training, enc_padding_mask, look_ahead_mask, dec_padding_mask):
        ...
        dec_output, attention_weights = self.decoder(
            tar, enc_output_img, training, look_ahead_mask, dec_padding_mask) # (batch, targ_len, d_model)
        final_output = self.final_layer(dec_output)  # (batch_size, tar_seq_len, target_vocab_size)
        return final_output, attention_weights
